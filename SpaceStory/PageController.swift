...
class PageController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        if let page = page {
            ...
            let attributatedString = NSMutableAttributedString(string: page.story.text)
            
            let paragraphStyle = NSMutableParagraphStyle()
            paragraphStyle.lineSpacing = 10
            
            attributatedString.addAttribute(NSParagraphStyleAttributeName, value: paragraphStyle, range: NSMakeRange(0, attributatedString.length))
            
            storyLabel.attributedText = attributatedString
    ...
    func loadFirstChoice(){
        if let page = page, firstCoice = page.firstChoice {
            let nextPage = firstCoice.page
